...
app.delete("/all", (req, res) => {
  if (typeof(req.query.masterKey) == 'string') {
    const given_key = req.query.masterKey;
    if (given_key === masterKey) {
      jokes = {}
      res.sendStatus(200);
    } else {
      res.json({error: 'masterkey did not match, jokes not deleted'});
    }
  } else {
    res.json({error: "no masterkey provided"});
  }
});
